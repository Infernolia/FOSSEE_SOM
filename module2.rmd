...
Thus we have successfully implemented Self Organizing Maps in R for dimensionality reduction using the Iris data set [@b1]. Using the grid we can see that there are predominantly 3 colors or clusters [green, blue and red] produced by the SOM, which correspond to the 3 original clusters Iris Setosa, Iris Versicolor and Iris Virginica of the Iris data set [@b1]. We experimented extensively with the size of the neuron grid, number of cells in visualization grid and the learning rate, but obtained the best results with the parameters specified in this code. Effective visualization is an important challenge of the SOM implementation as the weights needs to be expressed effectively to show the inherent groups or clusters in the data.
