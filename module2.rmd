...
In this module we will apply our knowledge from module 1 and implement SOM for dimensionality reduction for the Iris dataset [1].
...
# 2.7) Fastest BMU Implementation.
...
# 2.8) A function to encapsulate the entire creation, working and updating of SOM over the training period.
# "x" is the input and "input_grid" is the SOM grid that will be updated in each iteration.
...
# 3.2) Setting the seed to obtain reproducible results.
set.seed(222)
...
```{R echo=TRUE, message=FALSE, warning=FALSE}
# 5.1) Creating a function to visualize the SOM results by mapping the numbers to colors.
...
Thus we have successfully implemented Self Organizing Maps in R for dimensionality reduction using the Iris data set [1]. Using the grid we can see that there are predominantly 3 colors or clusters [green, blue and red] produced by the SOM, which correspond to the 3 original clusters Iris Setosa, Iris Versicolor and Iris Virginica of the Iris data set [1]. We experimented extensively with the size of the neuron grid, number of cells in visualization grid and the learning rate, but obtained the best results with the parameters specified in this code. Effective visualization is an important challenge of the SOM implementation as the weights needs to be expressed effectively to show the inherent groups or clusters in the data.
