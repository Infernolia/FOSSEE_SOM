...
SOM <- function(x, input_grid) {
  ...
  for(ne in 1:n_epochs)
  {
    ...
    {
      random_input_row <- as.vector(unlist(x[sample(1:nrow(x), size = 1, replace = F), ])) # Selecting random input row from given data set
      ...
      index_temp <- BMU(random_input_row, input_grid) # Finding best matching unit for given input row
      ...
      diff_grid=(sweep(input_grid[rn,],2,random_input_row))*-1 #A temporary matrix that stores the difference between the data point and the weights of the winning neuron & neighbours.
